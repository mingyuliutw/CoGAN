...
void AlterAdamSolver<Dtype>::AdamPreSolve() {
...
template <typename Dtype>
void AlterAdamSolver<Dtype>::ComputeUpdateValue(int param_id, Dtype rate) {
  const vector<Blob<Dtype>*>& net_params = this->net_->learnable_params();
  ...
  ///////////////////////////////////////////////////////////////////////////////////////////////
  // Ming-Yu Liu Edit Begin
  const vector<int>& net_params_update_base = this->net_->params_update_base();
  const vector<int>& net_params_update_bin  = this->net_->params_update_bin();
  const vector<bool>& net_has_params_update_base = this->net_->has_params_update_base();
  const vector<bool>& net_has_params_update_bin  = this->net_->has_params_update_bin();
  if( net_has_params_update_base[param_id] && net_has_params_update_bin[param_id] ) {
    ...
    if(bin != iter) {
      ...
      case Caffe::CPU:
        ...
        break;
      case Caffe::GPU:
        caffe_gpu_set<Dtype>(net_params[param_id]->count(), Dtype(0), net_params[param_id]->mutable_gpu_diff());
        break;
      default:
...
INSTANTIATE_CLASS(AlterAdamSolver);
REGISTER_SOLVER_CLASS(AlterAdam);
