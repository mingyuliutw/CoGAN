...
def make_gan_train_net(filename,opts):
	...
	for i in range(0,nLayers):
		...
		if(i==0):
			...
			if(i==nLayers-1):
				output_name_A = output_name + "_A"
				output_name_B = output_name + "_B"

				gan_lib.deconvolution_layer(fp,update_base,update_bin,input_name,output_name_A,neuron,kernel,pad,stride,weight_filter)
				gan_lib.sigmoid_layer(fp,output_name_A)

				gan_lib.deconvolution_layer(fp,update_base,update_bin,input_name,output_name_B,neuron,kernel,pad,stride,weight_filter)
				gan_lib.sigmoid_layer(fp,output_name_B)				
			else:
	...
	label_name_A = "labels_A"
	gan_lib.groundtruth_label_layer(fp,update_base,update_bin,label_name_A,num)
	label_name_B = "labels_B"
	gan_lib.groundtruth_label_layer(fp,update_base,update_bin,label_name_B,num)	
	label_name = "labels"
	gan_lib.concate_layer(fp,label_name_A,label_name_B,label_name,label_name)

	input_name_A  = output_name_A
	output_name_A = "data_and_fake_data_A"
	gan_lib.concate_layer(fp,"data_A",input_name_A,output_name_A,output_name_A)

	input_name_B  = output_name_B
	output_name_B = "data_and_fake_data_B"
	gan_lib.concate_layer(fp,"data_B",input_name_B,output_name_B,output_name_B)
	...
	for i in range(0,nLayers):
		input_name_A = output_name_A
		output_name_A= "conv%d" % i + "_A"
		input_name_B = output_name_B
		output_name_B= "conv%d" % i + "_B"		
		input_name = output_name
		...
		if(i==nLayers-1):
			...
			if(i==0):
				gan_lib.convolution_layer(fp,update_base,update_bin,input_name_A,output_name_A,neuron,kernel,pad,stride,weight_filter)
				gan_lib.convolution_layer(fp,update_base,update_bin,input_name_B,output_name_B,neuron,kernel,pad,stride,weight_filter)
				gan_lib.concate_layer(fp,output_name_A,output_name_B,output_name,output_name)
				input_name  = output_name
